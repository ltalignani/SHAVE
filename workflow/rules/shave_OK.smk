...
rule all:
    input:
        ...
        trimmed_fastqc = expand("results/00_Quality_Control/trimmed_fastqc/{sample}_sickle-trimmed_fastqc.html", sample=SAMPLE, aligner=ALIGNER),
...
rule multiqc:
    ...
    input:
        ...
        expand("results/00_Quality_Control/trimmed_fastqc/{sample}_sickle-trimmed_fastqc.html", sample=SAMPLE, aligner=ALIGNER),
...
rule trimmed_fastqc:
    ...
    priority: 1
    message:
        "Quality check after trimming for {wildcards.sample}"
    conda:
        FASTQC    
    input:
        reads = "results/01_Trimming/sickle/{sample}_sickle-trimmed_R1.fastq.gz",
        revreads = "results/01_Trimming/sickle/{sample}_sickle-trimmed_R2.fastq.gz"
    output:
        directory("results/00_Quality_Control/trimmed_fastqc/{sample}_sickle-trimmed_fastqc.html"),
    threads:
        CPUS
    params:
        "results/00_Quality_Control/trimmed_fastqc/"
    log:
        "results/11_Reports/trimmed_fastqc/{sample}_sickle-trimmed.fastqc.log"
    shell:
        "fastqc --threads {threads} --outdir={params} {input.fwdreads} {input.revreads} &>{log}"
