...
rule lofreq_indel_qualities:
    # Aim: Indels qualities. Can be used instead of GATK’s BQSR. 
...
###############################################################################
rule samtools_calmd:
    # Aim: Tool to generate the MD tag for SNP/indel calling w/o looking at the reference. It also generates at the same time the NM (number of mismatches) tag, which records the Levenshtein distance between the read and the reference.
